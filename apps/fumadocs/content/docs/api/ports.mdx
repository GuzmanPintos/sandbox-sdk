...
| Ascii Box      | Returns a public URL by default; protected mode keeps its token inside `request()`. |
| Railway        | Unsupported in the SDK; use `railway sandbox forward` locally.     |
| Tenki Sandbox  | Returns a public HTTPS preview URL on `tenki.sh`.                  |
